...
function create_validator() {
    ...
    for ((i = 0; i < size; i++)); do
        cp -r ${workspace}/keys/validator${i} ${workspace}/local/chain/
        cp -r ${workspace}/keys/bls${i} ${workspace}/local/chain/
    done
}
...
function prepare_config() {
    ...
    for ((i = 0; i < size; i++)); do
        for f in ${workspace}/local/chain/validator${i}/keystore/*; do
            cons_addr="0x$(cat ${f} | jq -r .address)"
            initHolders=${initHolders}","${cons_addr}
            fee_addr=${cons_addr}
        done

        mkdir -p ${workspace}/local/chain/node${i}
        cp ${workspace}/keys/password.txt ${workspace}/local/chain/node${i}/
        cp ${workspace}/local/chain/hardforkTime.txt ${workspace}/local/chain/node${i}/
        bbcfee_addrs=${fee_addr}
        powers="0x000001d1a94a2000" #2000000000000
        mv ${workspace}/local/chain/bls${i}/bls ${workspace}/local/chain/node${i}/ && rm -rf ${workspace}/local/chain/bls${i}
        vote_addr=0x$(cat ${workspace}/local/chain/node${i}/bls/keystore/*json | jq .pubkey | sed 's/"//g')
...
function initNetwork() {
    cd ${workspace}
    for ((i = 0; i < size; i++)); do
        mkdir ${workspace}/local/chain/node${i}/geth
        cp ${workspace}/keys/nodekey${i} ${workspace}/local/chain/node${i}/geth/nodekey
    done
    ${workspace}/bin/geth init-network --init.dir ${workspace}/local/chain --init.size=${size} --config ${workspace}/config.toml ${workspace}/genesis/genesis.json
    rm -rf ${workspace}/*chain.log*
    for ((i = 0; i < size; i++)); do
        sed -i -e '/"<nil>"/d' ${workspace}/local/chain/node${i}/config.toml
        mv ${workspace}/local/chain/validator${i}/keystore ${workspace}/local/chain/node${i}/ && rm -rf ${workspace}/local/chain/validator${i}

        cp ${workspace}/bin/geth ${workspace}/local/chain/node${i}/geth${i}
        # init genesis
        initLog=${workspace}/local/chain/node${i}/init.log
        if  [ $i -eq 0 ] ; then
                ${workspace}/bin/geth --datadir ${workspace}/local/chain/node${i} init --state.scheme ${stateScheme} --db.engine ${dbEngine} ${workspace}/genesis/genesis.json  > "${initLog}" 2>&1
        elif  [ $i -eq 1 ] ; then
            ${workspace}/bin/geth --datadir ${workspace}/local/chain/node${i} init --state.scheme path --db.engine pebble --multidatabase ${workspace}/genesis/genesis.json  > "${initLog}" 2>&1
        else
            ${workspace}/bin/geth --datadir ${workspace}/local/chain/node${i} init --state.scheme path --db.engine pebble ${workspace}/genesis/genesis.json  > "${initLog}" 2>&1
        fi
    done
}

function native_start() {
    PassedForkTime=`cat ${workspace}/local/chain/hardforkTime.txt|grep passedHardforkTime|awk -F" " '{print $NF}'`
